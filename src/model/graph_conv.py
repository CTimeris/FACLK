...
class GraphAggregateLayers(nn.Module):
    ...
    def compute_all_item_attn_scores(self, item_triplets, entity_embs):
        ...
        raw_scores = torch.sum(queries * keys, dim=-1) / math.sqrt(self._d_k) # [n_item_triplets, n_heads]  f
        raw_scores = self.leakyrelu(raw_scores)
    ...
    def _aggregate_cg(self, interaction_matrix, embeddings):
        agg_embs = torch.sparse.mm(interaction_matrix, embeddings)  
        return agg_embs
    
    def forward_cg(self, user_adj_mat, item_adj_mat, user_embs, item_embs, mess_dropout=False):
        ...
        user_emb_list_cg = [user_embs]
        item_emb_list_cg = [item_embs]
        ...
        for h in range(1, self._n_hops_cg+1):
            ...
            user_emb_list_cg.append(cur_layer_user_embs)
            item_emb_list_cg.append(cur_layer_item_embs)
        ...
        # user_final_embs_cg = torch.mean(torch.stack(user_emb_list_cg, dim=1), dim=1)
        # item_final_embs_cg = torch.mean(torch.stack(item_emb_list_cg, dim=1), dim=1)

        return user_final_embs_cg, item_final_embs_cg, user_emb_list_cg, item_emb_list_cg
    
    def _is_at_high_hop(self, h):
        return h + 1 < self._n_hops_kg + 1
    
    def _holistic_aggregate(self, kg_triplets, entity_embs, inter_mat, user_embs,h):
    ...
    def forward_holistic(self, kg_triplets, item_triplets, entity_embs, raw_scores, inter_mat, user_embs, mess_dropout=False):
        ...
        user_emb_list_kg = [cur_layer_user_embs]
        entity_emb_list = [cur_layer_entity_embs]
        ...
        for h in range(1, self._n_hops_kg+1):
            ...
            user_emb_list_kg.append(cur_layer_user_embs)
            entity_emb_list.append(cur_layer_entity_embs)
        ...
        user_final_embs_kg = torch.mean(torch.stack(user_emb_list_kg, dim=1), dim=1)
        entity_final_embs = torch.mean(torch.stack(entity_emb_list, dim=1), dim=1)

        return entity_final_embs, user_final_embs_kg, item_attn_final_embs, preference_embs, user_emb_list_kg, entity_emb_list
