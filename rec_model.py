...
class Rec(nn.Module):
    def __init__(self, args, dataset: Dataset):
        ...
        self.w_cg = args.w_cg
        self.w_kg = args.w_kg
        self.cor_user_cg = args.cor_user_cg
        self.cor_item_cg = args.cor_item_cg
        self.cor_user_kg = args.cor_user_kg
        self.cor_item_kg = args.cor_item_kg
        ...
        self.n_hops_cg = args.n_hops_cg
        self.n_hops_kg = args.n_hops_kg
        ...
        self._gnn = GraphAggregateLayers(args, dataset)  # graph_conv
    ...
    def _cg_dropout(self, user_adj_mat, item_adj_mat, rate=0.5):
        ...
        sampled_user_adj_mat = torch.sparse_coo_tensor(indices_user,
                                                        values_user, 
        ...
        sampled_item_adj_mat = torch.sparse_coo_tensor(indices_item_t,
                                                        values_item_t, 
    ...
    def _sparse_dropout(self, sparse_mat, rate=0.5):
        ...
        out = torch.sparse_coo_tensor(indices,
                                       values,
    ...
    def _aggregate_graphs(self, mess_dropout=False, node_dropout=False):
        ...
        entity_final_embs, user_final_embs_kg, \
            item_attn_final_embs, preference_embs, user_emb_list_kg, entity_emb_list = self._gnn.forward_holistic(kg_triplets,
                                                                               item_triplets,
                                                                               self._entity_embs,
                                                                               raw_scores,
                                                                               inter_mat,
        ...
        user_final_embs_cg, item_final_embs_cg, user_emb_list_cg, item_emb_list_cg = self._gnn.forward_cg(user_adj_mat,
                                                                                                          item_adj_mat,
                                                                                                          self._user_embs,
        ...
        return entity_final_embs, user_final_embs_kg, \
                user_final_embs_cg, item_final_embs_cg, item_attn_final_embs, preference_embs, user_emb_list_cg, item_emb_list_cg, user_emb_list_kg, entity_emb_list

    def forward(self, user_ids, item_ids, neg_item_ids, mess_dropout=False, node_dropout=False):
                entity_final_embs, user_final_embs_kg, user_final_embs_cg, \
         item_final_embs_cg, item_attn_final_embs, preference_embs, user_emb_list_cg, item_emb_list_cg, user_emb_list_kg, entity_emb_list = self._aggregate_graphs(mess_dropout, node_dropout)
        ...
        loss = self._object_func(user_ids, item_ids, neg_item_ids, entity_final_embs, user_final_embs_kg, user_final_embs_cg,
                                 preference_embs, item_final_embs_cg, item_attn_final_embs, user_emb_list_cg, item_emb_list_cg, user_emb_list_kg, entity_emb_list)
        
        return loss

    # item_embs_cg, item_embs_attn, item_embs_kg, user_embs_cg, user_embs_kg, user_preference_embs
    def _constrast_loss(self, item_final_embs_cg, item_attn_final_embs, item_final_embs_kg,
        ...
        user_total_emb = self._get_user_total_rep(user_final_embs_cg, user_final_embs_kg, preference_embs)
        item_total_emb = self._get_item_total_rep(item_final_embs_cg, item_final_embs_kg, item_attn_final_embs)

        item_pos_sim = _sim(item_total_emb, item_final_embs_cg)   
        item_neg_sim = _sim(item_total_emb, item_final_embs_kg)   
        user_pos_sim = _sim(user_total_emb, user_final_embs_cg)
        user_neg_sim = _sim(user_total_emb, user_final_embs_kg)
    ...
    def _get_user_total_rep(self, user_embs_cg, user_final_embs_kg, user_preference_embs):
        user_embs_total = self.w_cg * user_embs_cg + (1-self.w_cg) * user_final_embs_kg + self.w_kg * user_preference_embs
        return user_embs_total

    def _get_item_total_rep(self, item_embs_cg, item_final_embs_kg,  item_attn_embs):
        item_embs_total = self.w_cg * item_embs_cg + (1-self.w_cg) * item_final_embs_kg + self.w_kg * item_attn_embs
        return item_embs_total
    
    def _object_func(self, user_ids, item_ids, neg_item_ids, entity_final_embs, user_final_embs_kg, user_final_embs_cg,
                     preference_embs, item_final_embs_cg, item_attn_final_embs, user_emb_list_cg, item_emb_list_cg, user_emb_list_kg, entity_emb_list):
        ...
        # bpr loss
        user_embs_total = self._get_user_total_rep(user_embs_cg, user_embs_kg, user_preference_embs)
        item_embs_total = self._get_item_total_rep(item_embs_cg, item_embs_kg, item_embs_attn)
        neg_item_embs_total = self._get_item_total_rep(neg_item_embs_cg, neg_item_embs_kg, neg_item_embs_attn)
        ...
        # contrastive loss
        cl_loss = self._cl_decay * self._constrast_loss(item_embs_cg, item_embs_attn,  item_embs_kg,
                                                        user_embs_cg,  user_embs_kg, user_preference_embs)
        ...
        item_emb_list_kg = []
        for entity_emb in entity_emb_list:
            item_emb_list_kg.append(entity_emb[item_ids])
        cor_loss_u_cg = self.cor_user_cg * self.cor_loss(user_emb_list_cg, self.n_hops_cg)
        cor_loss_i_cg = self.cor_item_cg * self.cor_loss(item_emb_list_cg, self.n_hops_cg)
        cor_loss_u_kg = self.cor_user_kg * self.cor_loss(user_emb_list_kg, self.n_hops_kg)
        cor_loss_i_kg = self.cor_item_kg * self.cor_loss(item_emb_list_kg, self.n_hops_kg)

        cor_loss_u = cor_loss_u_cg + cor_loss_u_kg
        cor_loss_i = cor_loss_i_cg + cor_loss_i_kg
        loss = bpr_loss + cg_bpr_loss + cl_loss + emb_loss + cor_loss_u + cor_loss_i

        return loss, bpr_loss, cg_bpr_loss, cl_loss, emb_loss, cor_loss_u, cor_loss_i

    def generate_combined_embs(self):
        self.eval()
        entity_final_embs, user_final_embs_kg, \
            user_final_embs_cg, item_final_embs_cg, item_attn_final_embs, preference_embs, user_emb_list_cg, item_emb_list_cg, user_emb_list_kg, entity_emb_list = self._aggregate_graphs()

        user_embs_total = self._get_user_total_rep(user_final_embs_cg, user_final_embs_kg, preference_embs)
        item_embs_total = self._get_item_total_rep(item_final_embs_cg, entity_final_embs[:self._n_items], item_attn_final_embs)

        self.train()

        return user_embs_total, item_embs_total
    
    def rating_users_all_items(self, user_embs, item_embs):
        pred_scores_mat = torch.matmul(user_embs, item_embs.t())
        return pred_scores_mat.detach().cpu().numpy()

    def cor_loss(self, emb_list, layers):
        cor_loss = torch.zeros((1,)).to(self._device)
        layer_correlations = []

        for i in range(1, layers + 1):
            layer_correlations.append(emb_list[i].T.corrcoef().triu(diagonal=1).norm())

        layer_correlations_coef = (1 / torch.tensor(layer_correlations)) / torch.sum(1 / torch.tensor(layer_correlations))

        for i in range(1, layers+1):
            cor_loss += layer_correlations_coef[i - 1] * layer_correlations[i - 1]
        return cor_loss
