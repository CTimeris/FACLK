...
from model.rec_model import Rec
...
if __name__ == '__main__':
    ...
    handler = logging.FileHandler("res/{0}.txt".format(args.dataset))
    ...
    for epoch in range(start_epoch, args.n_epochs):
        ...
        while start < dataset.train_data.shape[0]:
            ...
            loss, bpr_loss, cg_bpr_loss, cl_loss, emb_loss, cor_u, cor_i = model.forward(user_ids,
                                                                           item_ids, 
                                                                           neg_item_ids, 
            ...
            loss_record['cor_u'].append(cor_u.item())
            loss_record['cor_i'].append(cor_i.item())
        ...
        cor_u = np.sum(loss_record['cor_u'])
        cor_i = np.sum(loss_record['cor_i'])

        t_e_train = time()
        t_cur_epoch = t_e_train - t_s_train
        tot_training_time += t_cur_epoch

        logger.info('Epoch: <%.2d>, Training time: %.2f s, Loss: %.4f (avg: %.4f) = bpr: %.3f + cgbpr: %.3f +  cl: %.3f + emb: %.3f + cor_u: %.3f + cor_i: %.3f'
                        %(epoch+1, t_cur_epoch, tot_loss, avg_loss, tot_bpr_loss, tot_cg_bpr_loss, tot_cl_loss, tot_emb_loss, cor_u, cor_i))
        # print("corr：{0}".format(cor_u+cor_i))
        if (epoch + 1) % args.show_steps == 0 or (epoch + 1) == 1:
            logger.info('********************************************************************************')
            t_s_eval = time()
            with torch.no_grad():
                user_embs, item_embs = model.generate_combined_embs()
                
                test_result = eval_ctr_topk(args, model, dataset, user_embs, item_embs)
